...
def format_date_filter(date_value, format_string='%B %d, %Y at %H:%M'):
    """
    Template filter to format dates whether they're strings or datetime objects
    """
...
def index():
    ...
    return render_template('index.html', banner=banner, products=products)
...
def order_confirmation(order_id):
    # Prevent admin users from viewing order confirmation
    if current_user.is_admin:
...
@login_required
def orders():
    # Prevent admin users from viewing user orders
    if current_user.is_admin:
...
def upload_payment_proof(order_id):
    # Prevent admin users from uploading payment proof
    if current_user.is_admin:
...
def admin_order_detail(order_id):
    ...
    # Fixed SQL query with explicit column order
    cur.execute("""
        SELECT 
            o.id, 
            o.total_amount, 
...
def admin_add_banner():
    ...
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            # Deactivate other banners if this one is set to active
            if active:
                cur = mysql.connection.cursor()
                cur.execute("UPDATE banners SET active = FALSE")
                mysql.connection.commit()
            
            cur.execute("INSERT INTO banners (title, image, active) VALUES (%s, %s, %s)", (title, filename, active))
            mysql.connection.commit()
            cur.close()
...
def admin_edit_banner(banner_id):
    ...
    if request.method == 'POST':
        ...
        if image_filename:
            cur.execute("UPDATE banners SET title = %s, image = %s, active = %s WHERE id = %s", 
                       (title, image_filename, active, banner_id))
        else:
            cur.execute("UPDATE banners SET title = %s, active = %s WHERE id = %s", 
                       (title, active, banner_id))
